...
Forest are complex areas, the mapping of such environment needs the use of different image processing methods. The extraction of "homogeneous" forest stands is at the interplay between different kinds of image processing methods (see Figure~\ref{fig:mindmap}).
...
%grow cyclic,
%level 1/.append style={level distance=5.5cm,sibling angle=72},
%level 2/.append style={level distance=3.6cm,sibling angle=45},
] 
node[concept, text width=4.75cm] {\textsc{Forest analysis} }
[clockwise from=90] 
child[concept color=green!50!black, level distance=7cm,sibling angle=72] { 
node[concept, text width=3cm] {Stand \\ segmentation} 
[clockwise from=150] 
child[concept color=green!50!black!75,level distance=3.7cm,sibling angle=60] { node[concept, text width=2.25cm] {Optical} } 
child[concept color=green!50!black!75,level distance=3.7cm,sibling angle=60] { node[concept, text width=2.25cm] {Lidar } }
child[concept color=green!50!black!75,level distance=3.7cm,sibling angle=60] { node[concept, text width=2.25cm] {Optical \& Lidar} } 
} 
child[concept color=blue!50!red, level distance=5.5cm,sibling angle=115] { 
node[concept, text width=3cm] {Classification} 
} 
child[concept color=red, level distance=5.5cm,sibling angle=75] {
node[concept, text width=3cm] {Dimension reduction}
[clockwise from=325] 
child[concept color=red!75,level distance=3.7cm,sibling angle=60] { node[concept, text width=2.25cm] {Feature extraction} }
child[concept color=red!75,level distance=3.7cm,sibling angle=60] { node[concept, text width=2.25cm] {Feature selection} }
}
child[concept color=orange, level distance=5.5cm,sibling angle=182] {
node[concept, text width=3cm] {Smoothing}
[clockwise from=267] 
child[concept color=orange!75,level distance=3.7cm,sibling angle=60] { node[concept, text width=2.25cm] {Local } }
child[concept color=orange!75,level distance=3.7cm,sibling angle=60] { node[concept, text width=2.25cm] {Global } }
}
child[concept color=blue, level distance=5.5cm,sibling angle=65] { 
node[concept, text width=3cm] {Segmentation} 
[clockwise from=320] 
child[concept color=blue!75,level distance=3.7cm,sibling angle=60] { node[concept, text width=2.25cm] {"Tradidional"} } 
child[concept color=blue!75,level distance=3.7cm,sibling angle=60] { node[concept, text width=2.25cm] {Superpixels} }
child[concept color=blue!75,level distance=3.7cm,sibling angle=120] { node[concept, text width=2.25cm] {Point cloud} }
};
\end{tikzpicture}
\caption{Processes involved in the analysis of forested areas.}
\label{fig:mindmap}
\end{figure}
...
In standard LC classification tasks, global methods are known to provide significantly more accurate results \citep{schindler2012overview} since contextual knowledge is integrated. This is all the more true for VHR remote sensing data, especially in case of a large number of classes (e.g., 10, \citep{isprs-archives-XLI-B4-11-2016}), but presents two disadvantages. For large datasets, their learning and inference steps are expensive to compute. Furthermore, parameters should often be carefully chosen for optimal performance, and authors that managed to alleviate the latter problem still report a significant computation cost \citep{Lucchi_ICCV2011}.
